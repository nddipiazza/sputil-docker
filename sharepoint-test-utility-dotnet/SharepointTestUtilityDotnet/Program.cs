    ...
    class Program
    {
        ...
        void exec()
        {
            ...
            foreach (Dictionary<string, object> action in actions)
            {
                ...
                else if (actionType.Equals("createGroup"))
                ...
                else if (actionType.Equals("deleteGroup"))
                ...
                else if (actionType.Equals("CreateTaxonomyGroup"))
                {
                    //string termStoreName = (string)action["TermStoreName"];
                    string termGroupName = (string)action["TermGroupName"];
                    using (ClientContext clientContext = getClientContext(removeHttpsFromUrl((string)action["SiteUrl"])))
                    {
                        TaxonomySession taxonomySession = TaxonomySession.GetTaxonomySession(clientContext);
                        TermStore termStore = taxonomySession.GetDefaultSiteCollectionTermStore();
                        TermGroup termGroup = termStore.GetSiteCollectionGroup(clientContext.Site, true);
                        termStore.CommitAll();
                        clientContext.ExecuteQuery();
                        clientContext.Load(termGroup, tg => tg.Id);
                        clientContext.ExecuteQuery();
                        res.Add(termGroup.Id.ToString());
                    }
                }
                else if (actionType.Equals("CreateTermSet"))
                {
                    //string termStoreName = (string)action["TermStoreName"];
                    string termGroupGuid = (string)action["TermGroupGuid"];
                    string termSetName = (string)action["TermSetName"];
                    int lcid = (int)action["Lcid"];
                    using (ClientContext clientContext = getClientContext(removeHttpsFromUrl((string)action["SiteUrl"])))
                    {
                        TaxonomySession taxonomySession = TaxonomySession.GetTaxonomySession(clientContext);
                        TermStore termStore = taxonomySession.GetDefaultSiteCollectionTermStore();
                        Guid termGroupGuidObj = new Guid(termGroupGuid);
                        TermGroup termGroup = termStore.GetGroup(termGroupGuidObj);
                        Guid newGuid = Guid.NewGuid();
                        TermSet termSetColl = termGroup.CreateTermSet(termSetName, newGuid, lcid);
                        clientContext.ExecuteQuery();
                        res.Add(newGuid.ToString());
                    }
                }
                else if (actionType.Equals("CreateTerm"))
                {
                    //string termStoreName = (string)action["TermStoreName"];
                    string termGroupGuid = (string)action["TermGroupGuid"];
                    string termSetGuid = (string)action["TermSetGuid"];
                    string termName = (string)action["TermName"];
                    string parentTermGuid = action.ContainsKey("ParentTermGuid") ? (string)action["ParentTermGuid"] : null;
                    int lcid = (int)action["Lcid"];
                    using (ClientContext clientContext = getClientContext(removeHttpsFromUrl((string)action["SiteUrl"])))
                    {
                        TaxonomySession taxonomySession = TaxonomySession.GetTaxonomySession(clientContext);
                        TermStore termStore = taxonomySession.GetDefaultSiteCollectionTermStore();
                        Guid termGroupGuidObj = new Guid(termGroupGuid);
                        Guid termSetGuidObj = new Guid(termSetGuid);
                        TermGroup termGroup = termStore.GetGroup(termGroupGuidObj);
                        TermSet termSetCol = termGroup.TermSets.GetById(termSetGuidObj);
                        Guid newGuid = Guid.NewGuid();
                        Term newTerm;
                        if (parentTermGuid == null)
                        {
                            newTerm = termSetCol.CreateTerm(termName, lcid, newGuid);
                        }
                        else
                        {
                            Guid parentTermGuidObj = new Guid(parentTermGuid);
                            Term parentTerm = termSetCol.GetTerm(parentTermGuidObj);
                            clientContext.Load(parentTerm, pt => pt.Id);
                            clientContext.ExecuteQuery();

                            parentTerm.CreateTerm(termName, lcid, newGuid);
                        }
                        clientContext.ExecuteQuery();
                        res.Add(newGuid.ToString());
                    }
                }
                else if (actionType.Equals("UpdateTerm"))
                {
                    //string termStoreName = (string)action["TermStoreName"];
                    string termGroupGuid = (string)action["TermGroupGuid"];
                    string termSetGuid = (string)action["TermSetGuid"];
                    string termName = (string)action["TermName"];
                    string termGuid = (string)action["TermGuid"];
                    using (ClientContext clientContext = getClientContext(removeHttpsFromUrl((string)action["SiteUrl"])))
                    {
                        TaxonomySession taxonomySession = TaxonomySession.GetTaxonomySession(clientContext);
                        TermStore termStore = taxonomySession.GetDefaultSiteCollectionTermStore();
                        Guid termGroupGuidObj = new Guid(termGroupGuid);
                        Guid termSetGuidObj = new Guid(termSetGuid);
                        Guid termGuidObj = new Guid(termGuid);
                        TermGroup termGroup = termStore.GetGroup(termGroupGuidObj);
                        TermSet termSetCol = termGroup.TermSets.GetById(termSetGuidObj);
                        Term term = termSetCol.GetTerm(termGuidObj);
                        term.Name = termName;
                        clientContext.ExecuteQuery();
                        res.Add("");
                    }
                }
                else if (actionType.Equals("RemoveTerm"))
                {
                    //string termStoreName = (string)action["TermStoreName"];
                    string termGroupGuid = (string)action["TermGroupGuid"];
                    string termSetGuid = (string)action["TermSetGuid"];
                ...
                else if (actionType.Equals("CreateListWithTaxonomyField"))
                {
                    using (ClientContext clientContext = getClientContext(removeHttpsFromUrl((string)action["SiteUrl"])))
                    {
                        Web web = clientContext.Web;
                        ...
                        Console.WriteLine("Created list - new GUID is {0}. Now creating the taxonomy field", list.Id);

                        string schemaTaxonomyField = "<Field Type='TaxonomyFieldType' Name='taxonomy' StaticName='taxonomy' DisplayName='taxonomy' Mult='TRUE' />";
                        ...
                        Guid termSetGuid = new Guid((string)action["TermSetGuid"]);
                        ...
                        taxonomyField.TermSetId = termSetGuid;
